...
- `README.md` — this file
- `rust/` — pure-Rust port (band-detection path; optional `slope_test`
  feature pulls in [`multitaper_rs`](https://github.com/preraulab/multitaper_toolbox)
  for the MTS-based branch). Validated against MATLAB R2025a on a
  synthetic-EEG test battery. See `rust/README.md`.

## Rust port

A Rust port lives in [`rust/`](rust/). It implements the band-detection
pipeline (broadband + high-frequency envelope detection with iterative
robust z-score), the flat-run mask, and the ±10σ outlier-noise
backstop. The `slope_test` Cargo feature additionally pulls in
`multitaper_rs` to provide the slope-test branch.

Cross-version parity is gated by a synthetic-EEG fixture battery
(pink noise + slow oscillation + spindle bursts + line noise +
motion-artifact spikes + flat runs) generated by
`DYNAM-O_DesktopApp/scripts/sim_eeg.m` and dumped to `.npy`. Per-variant
Rust↔MATLAB-no-slope sample-disagreement budgets:

| Variant | Budget (samples) |
|---|---|
| `sim_clean`  | ≤ 5   |
| `sim_motion` | ≤ 30  |
| `sim_flat`   | ≤ 5   |
...
```
cd rust
cargo test                        # band-detection parity + ground-truth
cargo test --features slope_test  # also runs the slope-test branch

...
% Rust fast path: when the caller wants the standard band-detection
% pipeline (no user exclusions, no diagnostic plots, no slope test),
% dispatch to detect_artifacts_mex. Matches MATLAB
% detect_artifacts(..., 'slope_test', false) to <=30 samples on the
% synthetic-EEG battery (see rust_bridge/validate_detect_artifacts_mex.m).
% slope_test=true stays on the MATLAB path below (needs the MTM
% spectrogram + log-log polyfit).
if exist('detect_artifacts_mex', 'file') == 3 ...
        && ~opts.slope_test ...
        && ~opts.return_filts_only ...
        && ~opts.diagnostic_plot ...
        && ~opts.histogram_plot ...
        && ~any(opts.isexcluded)
    rust_opts = struct( ...
        'hf_pass',          opts.hf_pass, ...
        'hf_crit',          opts.hf_crit, ...
        'bb_pass',          opts.bb_pass, ...
        'bb_crit',          opts.bb_crit, ...
        'hf_detrend',       opts.hf_detrend, ...
        'bb_detrend',       opts.bb_detrend, ...
        'smooth_duration',  opts.smooth_duration, ...
        'detrend_duration', opts.detrend_duration, ...
        'buffer_duration',  opts.buffer_duration, ...
        'zscore_method',    opts.zscore_method);
    band_mask = detect_artifacts_mex(double(data), Fs, rust_opts);
    if opts.buffer_duration > 0
        buf = round(opts.buffer_duration * Fs);
        band_mask = movmax(double(band_mask), [buf buf]) > 0;
    end
    artifacts = band_mask(:);
    return
end
